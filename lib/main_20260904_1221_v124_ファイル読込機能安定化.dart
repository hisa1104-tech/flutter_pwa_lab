...
void main() async {
  ...
  // 🔐 端末内暗号化のキー生成（共通パスワードをベースに32バイト化）
  // ※ 商品化の際は、ログインした医師のパスワードから動的に生成するように拡張可能
  const passwordSeed = 'X7p9W2r4K1mQ';
  final encryptionKey = Uint8List.fromList(
    utf8.encode(passwordSeed.padRight(32, '0')).sublist(0, 32),
  );

  await Hive.openBox('settings_box'); // アプリ設定（非個人情報）用

  try {
    // 🔐 患者データが入るBoxをAES-256で暗号化して開く
    await Hive.openBox('anes_box', encryptionCipher: HiveAesCipher(encryptionKey));
  } catch (e) {
    // 💡 暗号化されていない古いデータが残っている場合、エラーになるため一度リセット
    debugPrint('Encryption migration: Clearing old data...');
    await Hive.deleteBoxFromDisk('anes_box');
    await Hive.openBox('anes_box', encryptionCipher: HiveAesCipher(encryptionKey));
  }
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('SedeLog (β)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
...
class AnesthesiaDotPainter extends FlDotCirclePainter {
  ...
  void draw(Canvas canvas, FlSpot spot, Offset offset) {
    final paint = Paint()
      ..color = isVisible ? customColor : customColor.withOpacity(0.3) // 💡 明滅
...
class _MainRecordPageState extends State<MainRecordPage> {
  ...
  double _zoomMinutes = 0; // 👈 追加：ズーム（分）、0は全表示
  double _scrollOffset = 0; // 👈 追加：開始位置（分）
  ...
  // 💡 PDF用のフォントデータをキャッシュする変数
  pw.Font? _cachedFontRegular;
  pw.Font? _cachedFontBold;

  // 💡 現在編集中のデータのID（強調表示用）
  String? _editingId;
  bool _editingBlinkVisible = true;
  Timer? _blinkTimer;
  ...
  void initState() {
    ...
    _loadAllData();
    _loadFonts();    // 💡 フォントを事前に読み込んでキャッシュする
  }

  // 💡 アプリ起動時に一度だけフォントを読み込む（PDF生成の高速化）
  Future<void> _loadFonts() async {
    try {
      ...
      _cachedFontRegular = pw.Font.ttf(fontDataRegular);
      _cachedFontBold = pw.Font.ttf(fontDataBold);
      debugPrint('Font cache completed.');
    } catch (e) {
                ...
                Row(
                  children: [
                    ...
                    TextButton.icon(
                      ...
                      icon: Transform.flip(flipX: true, child: const Icon(Icons.exit_to_app, size: 16)),
  ...
  Future<void> _exportToFile() async {
    ...
    final fileName = 'beta_${id}_${name}_$dateStr.json';
  ...
  Future<void> _importFromFile() async {
    try {
      // 💡 方法 1: HTMLの input 要素を直接使用（iPad/Web専用の最適化）
      // これにより、カメラメニューを回避して直接ファイルブラウザを開きます。
      final html.FileUploadInputElement uploadInput = html.FileUploadInputElement();
      uploadInput.accept = '.json'; // JSONファイルのみを許可
      uploadInput.click(); // プログラム的にクリックしてダイアログを表示
      ...
      uploadInput.onChange.listen((e) {
        ...
        reader.onLoadEnd.listen((e) {
          try {
            final String jsonStr = reader.result as String;
            final data = jsonDecode(jsonStr) as Map<String, dynamic>;
            
            setState(() {
              _applyDataMap(data);
              _saveAllData(); // LocalStorageにも保存
            });

            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('患者データを復元しました')),
              );
            }
          } catch (err) {
            debugPrint('JSON解析エラー: $err');
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('ファイルの解析に失敗しました。正しいJSON形式か確認してください。')),
              );
            }
          }
        });
      ...
      debugPrint('ファイル読込エラー: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('ファイル読込機能の起動に失敗しました')),
        );
      }
    }
  }
  ...
  Future<void> _exportSettings() async {
    ...
    final jsonStr = jsonEncode(data);
    final dateStr = DateFormat('yyyyMMdd').format(DateTime.now());
    final fileName = 'beta_anes_settings_$dateStr.json';
  ...
  Future<void> _importSettings() async {
    try {
      // 💡 HTMLの input 要素を直接使用（iPad/Web専用の最適化）
      final html.FileUploadInputElement uploadInput = html.FileUploadInputElement();
      ...
      uploadInput.onChange.listen((e) {
        ...
        reader.onLoadEnd.listen((e) async {
          try {
            ...
            debugPrint('設定解析エラー: $err');
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('設定ファイルの解析に失敗しました。')),
              );
            }
          }
        });
  ...
  void _startBlinking() {
    ...
    _blinkTimer = Timer.periodic(const Duration(milliseconds: 600), (t) {
      setState(() => _editingBlinkVisible = !_editingBlinkVisible);
    });
  }
...
// 🌟 局所麻酔剤の自由入力用ポップアップダイアログ
        ...
        return const AlertDialog(
          content: Column(
            ...
            children: [
              ...
              SizedBox(height: 20),
              Text("PDFを作成しています...", style: TextStyle(fontSize: 14)),
              Text("しばらくお待ちください", style: TextStyle(fontSize: 12, color: Colors.grey)),
            ],
          ),
        );
      },
  ...
  void _showHelpDialog() {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('操作方法', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
          content: SingleChildScrollView(
            child: Column(
              ...
              children: const [
                ...
                Text('・左上の「患者アイコン」をタップするとメニュー（新規症例、データ出力、復元、設定）が開きます。', style: TextStyle(fontSize: 12)),
                Text('・「バイタル入力」ボタンからsBP/dBP/HR/SpO2を一括入力できます。', style: TextStyle(fontSize: 12)),
                SizedBox(height: 10),
                Text('【トレンド・タイムライン・記録一覧ログ】', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                Text('・薬剤名ラベルをタップすると、その行を一時的に非表示にできます。', style: TextStyle(fontSize: 12)),
                Text('・非表示にした行は「非表示にした行をすべて再表示」ボタンで復元できます。', style: TextStyle(fontSize: 12)),
                Text('・記録一覧ログから入力内容の修正ができます。', style: TextStyle(fontSize: 12)),
                SizedBox(height: 10),
                ...
                SizedBox(height: 10),
                Text('【その他】', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                Text('・本アプリはiPad Air 11 inchで最適化されるようプログラムされています。他の端末では表示が崩れる可能性があります。', style: TextStyle(fontSize: 12)),
                Text('・バグ等の連絡先：xxxxxxxxxxxxxx', style: TextStyle(fontSize: 12)),
              ],
            ),
          ),
          actions: [
  ...
  Future<void> _generatePdf() async {
    _showPdfLoadingDialog(); // 💡 PDF作成中ダイアログを表示
    try {
      ...
      // 💡 キャッシュがあればそれを使い、なければその場で読み込む（予備用）
      ...
      final pw.Font fontRegular = _cachedFontRegular!;
      final pw.Font fontBold = _cachedFontBold!;
...
// =========================================================================
                        ...
                        children: [
                          ...
                          pw.SizedBox(
                            ...
                            child: pw.RichText(
                              text: pw.TextSpan(
                                children: [
                                  pw.TextSpan(text: '手術日: ', style: pw.TextStyle(font: fontBold, fontSize: 9)),
                                  pw.TextSpan(
                                    text: (_events.firstWhere((e) => e.id == 'enter').time != null)
                                        ? DateFormat('yyyy/MM/dd').format(_events.firstWhere((e) => e.id == 'enter').time!)
      ...
      final name = _pNameCtrl.text.isEmpty ? 'no-name' : _pNameCtrl.text;
      final dateStr = DateFormat('yyyyMMdd').format(DateTime.now());
      final fileName = 'beta_AnesRecord_${id}_${name}_$dateStr.pdf';
                ...
                children: [
                  ...
                  StreamBuilder(
                    ...
                    builder: (context, snapshot) {
                      return Text(
                        '現在時刻: ${DateFormat('HH:mm:ss').format(DateTime.now())}',
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      // 🔘 NOWボタン
                      ElevatedButton(
                        onPressed: () => setDialogState(() => targetTime = DateTime.now()),
                        style: ElevatedButton.styleFrom(
  ...
  */
  ...
  List<Widget> _getEventPins(double minMinutes, double maxMinutes, double width) {
    if (_startTime == null || maxMinutes <= minMinutes) return [];
    final double range = maxMinutes - minMinutes;
    if (range <= 0) return [];
    ...
    return _events
        ...
        .map((e) {
          double m = e.time!.difference(_startTime!).inSeconds.toDouble() / 60;
          return Positioned(
            left: (width * ((m - minMinutes) / range)).clamp(0.0, width) - 8,
            top: 0,
            bottom: 0,
            child: Align(
              ...
              child: InkWell(
                ...
                child: AnimatedScale(
                  ...
                  scale: _editingId == e.id ? 1.5 : 1.0,
                  child: AnimatedOpacity(
                    duration: const Duration(milliseconds: 300),
                    opacity: (_editingId == e.id && !_editingBlinkVisible) ? 0.3 : 1.0,
                    child: Text(
                      e.symbol,
                      style: TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.bold,
                        color: _editingId == e.id ? Colors.orange : e.activeColor,
                      ),
                    ),
                  ),
                ),
  ...
  List<Widget> _getCombinedIvAndRemarkPins(double minMinutes, double maxMinutes, double width) {
    if (_startTime == null || maxMinutes <= minMinutes) return [];
    final double range = maxMinutes - minMinutes;
    if (range <= 0) return [];
        ...
        Positioned(
          ...
          child: Align(
            ...
            child: InkWell(
              ...
              child: AnimatedScale(
                duration: const Duration(milliseconds: 300),
                scale: _editingId == iv.id ? 1.5 : 1.0,
                child: AnimatedOpacity(
                  duration: const Duration(milliseconds: 300),
                  opacity: (_editingId == iv.id && !_editingBlinkVisible) ? 0.3 : 1.0,
                  child: Text(
                    'PV',
                    style: TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.bold,
                      color: _editingId == iv.id ? Colors.orange : color,
                    ),
                  ),
                ),
              ),
    ...
    // 2. リマークスピン
    final visibleRemarks = _remarkLogs.where((rm) {
      double m = rm.time.difference(_startTime!).inSeconds.toDouble() / 60;
      return m >= minMinutes && m <= maxMinutes;
    });

    for (var rm in visibleRemarks) {
      double m = rm.time.difference(_startTime!).inSeconds.toDouble() / 60;
      pins.add(
        Positioned(
          left: (width * ((m - minMinutes) / range)).clamp(0.0, width) - 4,
          top: 0,
          bottom: 0,
          child: Align(
            alignment: Alignment.center,
            child: InkWell(
              ...
              child: AnimatedScale(
                duration: const Duration(milliseconds: 300),
                scale: _editingId == rm.id ? 1.5 : 1.0,
                child: AnimatedOpacity(
                  duration: const Duration(milliseconds: 300),
                  opacity: (_editingId == rm.id && !_editingBlinkVisible) ? 0.3 : 1.0,
                  child: Text(
                    '${rm.number}',
                    style: TextStyle(
                      fontSize: 12.0,
                      fontWeight: FontWeight.bold,
                      color: _editingId == rm.id ? Colors.orange : Colors.orange,
                      backgroundColor: _editingId == rm.id ? Colors.yellow.withOpacity(0.3) : null,
                    ),
                  ),
                ),
              ),
  ...
  List<Widget> _getInfusionGraphics(
    ...
    if (_startTime == null || maxMinutes <= minMinutes) return elements;
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      // クリップ処理
      if (startM < minMinutes) startM = minMinutes;
      if (endM > maxMinutes) endM = maxMinutes;
      if (startM >= endM) continue;

      double left = width * ((startM - minMinutes) / range);
      double lineWidth = (width * ((endM - minMinutes) / range)) - left;
    ...
    for (var pt in points) {
      ...
      double leftPosition = width * ((m - minMinutes) / range);
        ...
        Positioned(
          ...
          child: Align(
            ...
            child: InkWell(
              ...
              child: AnimatedScale(
                duration: const Duration(milliseconds: 300),
                scale: _editingId == pt.id ? 1.5 : 1.0,
                child: AnimatedOpacity(
                  duration: const Duration(milliseconds: 300),
                  opacity: (_editingId == pt.id && !_editingBlinkVisible) ? 0.3 : 1.0,
                  child: pt.isStop
                      ? Text(
                    '┃',
                    style: TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.bold,
                      color: _editingId == pt.id ? Colors.orange : color,
                    ),
                  )
                      : Text(
                    displayVal,
  ...
  List<Widget> _getBolusPins(
    ...
    List<Widget> pins = [];
    if (_startTime == null || maxMinutes <= minMinutes) return pins;
    double range = maxMinutes - minMinutes;
    ...
    if (drugFilter == '輸液' || drugFilter == _selectedFluidType) {
      double startMin =
          targets.first.time.difference(_startTime!).inSeconds.toDouble() / 60;
      double endMin =
          targets.last.time.difference(_startTime!).inSeconds.toDouble() / 60;

      // クリップ処理
      if (startMin < minMinutes) startMin = minMinutes;
      if (endMin > maxMinutes) endMin = maxMinutes;

      if (startMin < endMin) {
        double startX = (width * ((startMin - minMinutes) / range)).clamp(0.0, width);
        double endX = (width * ((endMin - minMinutes) / range)).clamp(0.0, width);
        ...
        Positioned(
          ...
          child: Align(
            ...
            child: InkWell(
              ...
              child: AnimatedScale(
                duration: const Duration(milliseconds: 300),
                scale: _editingId == b.id ? 1.5 : 1.0,
                child: AnimatedOpacity(
                  duration: const Duration(milliseconds: 300),
                  opacity: (_editingId == b.id && !_editingBlinkVisible) ? 0.3 : 1.0,
                  child: Text(
                    displayAmount,
                    style: TextStyle(
                      fontSize: 12,
                      fontWeight: FontWeight.bold,
                      color: _editingId == b.id ? Colors.orange : color,
                      backgroundColor:
  ...
  List<Widget> _getDynamicCustomBolusPins(
    ...
    for (var b in targets) {
      ...
      pins.add(
        Positioned(
          left: (width * ((m - minMinutes) / range)).clamp(0.0, width) - 6,
          top: 0,
          bottom: 0,
          child: Align(
            alignment: Alignment.center,
  ...
  LineChartData _mainChartData() {
    ...
    double totalMaxMinutes = 30.0;

    // 💡 グラフ全体の表示幅を決定するために、全データの最大時刻を網羅的にチェックします
    if (_startTime != null) {
      // 1. バイタルレコード
      for (var r in _records) {
        double m = r.dateTime.difference(_startTime!).inMinutes.toDouble();
        if (r.sbp > computedMaxY) computedMaxY = r.sbp + 20;
        if (m > totalMaxMinutes) totalMaxMinutes = m + 5;
      }
      // 2. イベントスタンプ
      for (var e in _events) {
        if (e.time != null) {
          double em = e.time!.difference(_startTime!).inMinutes.toDouble();
          if (em > totalMaxMinutes) totalMaxMinutes = em + 5;
        }
      }
      // 3. 点滴・持続投与
      for (var key in _infusionMap.keys) {
        if (_infusionMap[key]!.isNotEmpty) {
          double im = _infusionMap[key]!.last.time.difference(_startTime!).inMinutes.toDouble();
          if (im > totalMaxMinutes) totalMaxMinutes = im + 5;
        }
      }
      // 4. 単回投与(Bolus)
      for (var b in _bolusLogs) {
        double bm = b.time.difference(_startTime!).inMinutes.toDouble();
        if (bm > totalMaxMinutes) totalMaxMinutes = bm + 5;
      }
      // 5. リマークス（コメント）
      for (var rm in _remarkLogs) {
        double rmm = rm.time.difference(_startTime!).inMinutes.toDouble();
        if (rmm > totalMaxMinutes) totalMaxMinutes = rmm + 5;
      }
      // 6. ルート確保(IV)
      for (var iv in _ivRecords) {
        double ivm = iv.time.difference(_startTime!).inMinutes.toDouble();
        if (ivm > totalMaxMinutes) totalMaxMinutes = ivm + 5;
      }
    }

    // 💡 ズームが有効ならその範囲、無効（全表示）なら全体範囲を表示
    double visibleMinX = (_zoomMinutes > 0) ? _scrollOffset : 0.0;
    double visibleMaxX = (_zoomMinutes > 0) ? _scrollOffset + _zoomMinutes : totalMaxMinutes;
    ...
    if (_startTime != null && _records.isNotEmpty) {
      // 💡 描画前に必ず時間順でソート（あとから過去のデータを入力した場合の「線の逆走」を防止）
      _records.sort((a, b) => a.dateTime.compareTo(b.dateTime));
    ...
    double visibleRange = visibleMaxX - visibleMinX;
    double interval = 5.0;
    if (visibleRange >= 180) {
      interval = 30.0;
    } else if (visibleRange >= 120) {
      interval = 20.0;
        ...
        touchCallback: (FlTouchEvent event, LineTouchResponse? response) {
          ...
          if (event is FlTapUpEvent) {
            ...
            if (index >= 0 && index < _records.length) {
              _showVitalEditDialog(_records[index]);
            }
          }
        },
              ...
              FlDotData(
                ...
                getDotPainter: (spot, percent, barData, index) {
                  return FlDotCirclePainter(
                    radius: 5,
                    color: barData.color ?? Colors.red,
                    strokeWidth: 2,
        ...
        bottomTitles: AxisTitles(
          sideTitles: SideTitles(
            ...
            getTitlesWidget: (value, meta) {
              ...
              // 💡 「全表示」(_zoomMinutes == 0)の時だけ、右端ギリギリの目盛りを非表示にして重なりを防止
              // 固定ズーム(30分〜180分)の時は最後まで表示する
              if (_zoomMinutes == 0 && value > visibleMaxX - (interval * 0.2)) {
              ...
              return SideTitleWidget(
                ...
                child: Text(
                  DateFormat('HH:mm').format(
                    _startTime!.add(Duration(minutes: value.toInt())),
                  ),
        ...
        LineChartBarData(
          ...
          dotData: FlDotData(
            ...
            getDotPainter: (s, p, b, i) {
              final bool isEditing = (_editingId != null && i < _records.length && _records[i].id == _editingId);
              return AnesthesiaDotPainter(
                type: 'sbp',
                customColor: isEditing ? Colors.yellow : Colors.red,
                customSize: isEditing ? 16 : 10,
                isVisible: isEditing ? _editingBlinkVisible : true,
              );
            },
          ),
        ),
        ...
        LineChartBarData(
          ...
          dotData: FlDotData(
            ...
            getDotPainter: (s, p, b, i) {
              ...
              return AnesthesiaDotPainter(
                type: 'hr',
                customColor: isEditing ? Colors.yellow : Colors.green,
                customSize: isEditing ? 14 : 6,
                isVisible: isEditing ? _editingBlinkVisible : true,
              );
            },
          ),
        ...
        LineChartBarData(
          ...
          dotData: FlDotData(
            ...
            getDotPainter: (s, p, b, i) {
              ...
              return AnesthesiaDotPainter(
                type: 'spo2',
                customColor: isEditing ? Colors.yellow : Colors.cyan,
                customSize: isEditing ? 14 : 6,
                isVisible: isEditing ? _editingBlinkVisible : true,
              );
            },
  ...
  void _showCustomKeypadDialog() {
    DateTime entryTime = DateTime.now(); // 👈 入力時刻を保持
    String sbp = '';
    String dbp = '';
    String hr = '';
    String spo2 = '';
    ...
    showDialog(
      context: context,
      barrierColor: Colors.black.withOpacity(0.4), // 💡 背景を少し暗めに
      builder: (BuildContext context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            ...
            return AlertDialog(
              alignment: const Alignment(0.85, 0.0), // 👈 右側エリア（ログ＋パネル）の中央付近
              title: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('バイタル入力'),
                  Row(
                    children: [
                      IconButton(
                        onPressed: () => setDialogState(() => entryTime = entryTime.subtract(const Duration(minutes: 1))),
                        icon: const Icon(Icons.remove_circle_outline, size: 18, color: Colors.blueGrey),
                        padding: EdgeInsets.zero,
                      ...
                      const SizedBox(width: 8),
                      IconButton(
                        onPressed: () => setDialogState(() => entryTime = entryTime.add(const Duration(minutes: 1))),
                        icon: const Icon(Icons.add_circle_outline, size: 18, color: Colors.blueGrey),
                        padding: EdgeInsets.zero,
                        constraints: const BoxConstraints(),
                      ),
                    ],
                              ...
                              children: row
                                  ...
                                  child: Padding(
                                    ...
                                    child: ElevatedButton(
                                      ...
                                      child: k == 'NEXT'
                                          ? const Icon(Icons.arrow_forward, size: 18) // 👈 アイコンに変更
                                          : Text(
                                        k,
                                        style: const TextStyle(
                                          fontSize: 15,
  ...
  Widget _zoomButton(String label, double mins) {
    bool isSelected = _zoomMinutes == mins;
    return Padding(
      ...
      child: InkWell(
        onTap: () => setState(() {
          // 💡 スマート・ズーム & センター維持ロジック
          if (mins > 0) {
            // 1. 現在の全データの最大時刻を計算（データの右端を特定）
            double currentTotalMaxX = 30.0;
            if (_startTime != null) {
              for (var r in _records) {
                double m = r.dateTime.difference(_startTime!).inMinutes.toDouble();
                if (m > currentTotalMaxX) currentTotalMaxX = m + 5;
              }
              for (var e in _events) {
                if (e.time != null) {
                  double em = e.time!.difference(_startTime!).inMinutes.toDouble();
                  if (em > currentTotalMaxX) currentTotalMaxX = em + 5;
                }
              }
              for (var key in _infusionMap.keys) {
                if (_infusionMap[key]!.isNotEmpty) {
                  double im = _infusionMap[key]!.last.time.difference(_startTime!).inMinutes.toDouble();
                  if (im > currentTotalMaxX) currentTotalMaxX = im + 5;
                }
              }
              for (var b in _bolusLogs) {
                double bm = b.time.difference(_startTime!).inMinutes.toDouble();
                if (bm > currentTotalMaxX) currentTotalMaxX = bm + 5;
              }
              for (var rm in _remarkLogs) {
                double rmm = rm.time.difference(_startTime!).inMinutes.toDouble();
                if (rmm > currentTotalMaxX) currentTotalMaxX = rmm + 5;
              }
            }

            if (_zoomMinutes > 0) {
              // 2. すでにズーム中の場合：現在の「中心時刻」を維持する
              // 現在の表示幅の中央を計算
              double currentCenter = _scrollOffset + (_zoomMinutes / 2);
              // 新しいズーム幅(mins)において、その中心時刻を表示するためのオフセットを計算
              _scrollOffset = (currentCenter - (mins / 2)).clamp(0.0, (currentTotalMaxX - mins).clamp(0.0, currentTotalMaxX));
            } else {
              // 3. 全表示からズームへの切り替え：最新データ（右端）を表示
              _scrollOffset = (currentTotalMaxX - mins).clamp(0.0, currentTotalMaxX);
            }
          } else {
            // 全表示に戻すときはオフセットをリセット
            _scrollOffset = 0;
      ...
      body: SafeArea(
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            double totalMaxX = 30.0; // 💡 タイムライン側も同様に30分を最小単位に
            if (_startTime != null) {
              for (var r in _records) {
                double m = r.dateTime
              ...
              for (var e in _events) {
                if (e.time != null) {
                  double em = e.time!
                      ...
                      .inMinutes
                      .toDouble();
                  if (em > totalMaxX) totalMaxX = em + 5;
                }
              }
              for (var key in _infusionMap.keys) {
                if (_infusionMap[key]!.isNotEmpty) {
                  double im = _infusionMap[key]!.last.time
                      ...
                      .inMinutes
                      .toDouble();
                  if (im > totalMaxX) totalMaxX = im + 5;
                }
              }
              for (var b in _bolusLogs) {
                double bm = b.time.difference(_startTime!).inMinutes.toDouble();
                if (bm > totalMaxX) totalMaxX = bm + 5;
              }
              // 💡 リマークス(Remarks)も含める
              for (var rm in _remarkLogs) {
                double rmm = rm.time.difference(_startTime!).inMinutes.toDouble();
                if (rmm > totalMaxX) totalMaxX = rmm + 5;
              }
              // 💡 ルート確保(IV)も含める
              for (var iv in _ivRecords) {
                double ivm = iv.time.difference(_startTime!).inMinutes.toDouble();
                if (ivm > totalMaxX) totalMaxX = ivm + 5;
              }
            }

            // 💡 ズーム・スクロールの計算
            if (_zoomMinutes > 0) {
              if (_scrollOffset > totalMaxX - _zoomMinutes) {
                _scrollOffset = (totalMaxX - _zoomMinutes).clamp(0, totalMaxX);
              }
            } else {
              _scrollOffset = 0;
            }
            double visibleMinX = _zoomMinutes > 0 ? _scrollOffset : 0;
            double visibleMaxX = _zoomMinutes > 0 ? _scrollOffset + _zoomMinutes : totalMaxX;
                        ...
                        child: PopupMenuButton<String>(
                          ...
                          itemBuilder: (context) => [
                            ...
                            PopupMenuItem(
                              ...
                              child: Row(
                                children: [
                                  const Icon(Icons.output, color: Colors.blueGrey, size: 18),
                                  const SizedBox(width: 8),
                                  const Text('患者データ出力', style: TextStyle(fontSize: 13)),
                                ],
                              ),
                            ),
                            PopupMenuItem(
                              ...
                              child: Row(
                                children: [
                                  Transform.flip(flipX: true, child: const Icon(Icons.exit_to_app, color: Colors.blueGrey, size: 18)),
                                  const SizedBox(width: 8),
                                  const Text('患者データ読込', style: TextStyle(fontSize: 13)),
                                ],
                              ),
                            ),
                            const PopupMenuItem(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  SizedBox(width: 8),
                                  Text('設定', style: TextStyle(fontSize: 13)),
                                ],
                              ),
                            ),
                            const PopupMenuDivider(),
                            const PopupMenuItem(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  SizedBox(width: 8),
                                  Text('操作方法', style: TextStyle(fontSize: 13)),
                                ],
                              ),
                            ),
                          ],
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Padding(
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                            ...
                                            Row(
                                              children: [
                                                const Text('ズーム:', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
                                                const SizedBox(width: 4),
                                                _zoomButton('全表示', 0),
                                                _zoomButton('30分', 30),
                                                _zoomButton('60分', 60),
                                                _zoomButton('120分', 120),
                                                _zoomButton('180分', 180),
                                              ],
                                            ),
                                          ],
                                        ),
                                      ...
                                      // 💡 ズーム・スクロールUIの整理：スライダーのみ表示
                                      if (_zoomMinutes > 0)
                                        Padding(
                                          padding: const EdgeInsets.only(right: 15, left: 70),
                                          child: Row(
                                            children: [
                                              const Text('スクロール:', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
                                              Expanded(
                                                child: SliderTheme(
                                                  ...
                                                  child: Slider(
                                                    value: _scrollOffset,
                                                    min: 0,
                                                    max: (totalMaxX - _zoomMinutes).clamp(0, totalMaxX),
                                                    onChanged: (val) => setState(() => _scrollOffset = val),
                                                  ),
                                                ),
                                              ),
                                            ],
                                                  ...
                                                  child: ListView(
                                                    ...
                                                    children: [
                                                      ...
                                                      _buildTimelineRow(
                                                        ...
                                                        maxMinutes: visibleMaxX - visibleMinX,
                                                        children: _getEventPins(visibleMinX, visibleMaxX, chartW),
                                                      ),
                                                      _buildTimelineRow(
                                                        label: 'リマークス/PV',
                                                        rowKey: 'remark', // 👈 新しく追加した引数
                                                        maxMinutes: visibleMaxX - visibleMinX,
                                                        children: _getCombinedIvAndRemarkPins(visibleMinX, visibleMaxX, chartW),
                                                      ),
                                                      ...
                                                      if (_presetVisibleRows.contains('O2') && !_hiddenRowKeys.contains('O2'))
                                                        _buildTimelineRow(
                                                          ...
                                                          rowKey: 'O2',
                                                          maxMinutes: visibleMaxX - visibleMinX,
                                                          children: _getInfusionGraphics('O2', visibleMinX, visibleMaxX, chartW, Colors.blue),
                                                      ...
                                                      if ((_showN2oRow || _presetVisibleRows.contains('N2O')) && !_hiddenRowKeys.contains('N2O'))
                                                        _buildTimelineRow(
                                                          ...
                                                          rowKey: 'N2O',
                                                          maxMinutes: visibleMaxX - visibleMinX,
                                                          children: _getInfusionGraphics('N2O', visibleMinX, visibleMaxX, chartW, Colors.lightBlue.shade300),
                                                      ...
                                                      if ((_showDexRow || _presetVisibleRows.contains('Dex')) && !_hiddenRowKeys.contains('Dex'))
                                                        _buildTimelineRow(
                                                          ...
                                                          rowKey: 'Dex',
                                                          maxMinutes: visibleMaxX - visibleMinX,
                                                          children: _getInfusionGraphics('Dex', visibleMinX, visibleMaxX, chartW, Colors.orange),
                                                          bgColor: Colors.orange.withOpacity(0.01),
                                                        ),

                                                      if (_presetVisibleRows.contains('PropofolCiv') && !_hiddenRowKeys.contains('PropofolCiv'))
                                                        _buildTimelineRow(
                                                          label: 'Propofol [$_propofolInfUnit]',
                                                          rowKey: 'PropofolCiv',
                                                          maxMinutes: visibleMaxX - visibleMinX,
                                                          children: _getInfusionGraphics('PropofolInf', visibleMinX, visibleMaxX, chartW, Colors.purple),
                                                      ...
                                                      if (_presetVisibleRows.contains('PropofolIv') && !_hiddenRowKeys.contains('PropofolIv'))
                                                        _buildTimelineRow(
                                                          ...
                                                          rowKey: 'PropofolIv',
                                                          maxMinutes: visibleMaxX - visibleMinX,
                                                          children: _getBolusPins('Propofol', visibleMinX, visibleMaxX, chartW, Colors.deepPurple.shade400),
                                                          bgColor: Colors.purple.withOpacity(0.01),
                                                        ),
                                                      ...
                                                      if (_presetVisibleRows.contains('Midazolam') && !_hiddenRowKeys.contains('Midazolam'))
                                                        _buildTimelineRow(
                                                          ...
                                                          rowKey: 'Midazolam',
                                                          maxMinutes: visibleMaxX - visibleMinX,
                                                          children: _getBolusPins('Midazolam', visibleMinX, visibleMaxX, chartW, Colors.teal),
                                                      ...
                                                      if ((_showAcerioRow || _presetVisibleRows.contains('Acerio')) && !_hiddenRowKeys.contains('Acerio'))
                                                        _buildTimelineRow(
                                                          ...
                                                          rowKey: 'Acerio',
                                                          maxMinutes: visibleMaxX - visibleMinX,
                                                          children: _getBolusPins('アセリオ', visibleMinX, visibleMaxX, chartW, Colors.orange.shade700),
                                                          bgColor: Colors.orange.withOpacity(0.01),
                                                        ),
                                                      ...
                                                      if ((_showRopionRow || _presetVisibleRows.contains('Ropion')) && !_hiddenRowKeys.contains('Ropion'))
                                                        _buildTimelineRow(
                                                          ...
                                                          rowKey: 'Ropion',
                                                          maxMinutes: visibleMaxX - visibleMinX,
                                                          children: _getBolusPins('ロピオン', visibleMinX, visibleMaxX, chartW, Colors.brown),
                                                      ...
                                                      if (_presetVisibleRows.contains('LA') && !_hiddenRowKeys.contains('LA'))
                                                        _buildTimelineRow(
                                                          ...
                                                          maxMinutes: visibleMaxX - visibleMinX,
                                                          children: [
                                                            ..._getBolusPins(_selectedLaDrug, visibleMinX, visibleMaxX, chartW, Colors.indigo.shade800),
                                                            ..._getBolusPins('LA', visibleMinX, visibleMaxX, chartW, Colors.indigo.shade800),
                                                          ],
                                                      ...
                                                      ...customDrugNames
                                                        ...
                                                        return _buildTimelineRow(
                                                          ...
                                                          maxMinutes: visibleMaxX - visibleMinX,
                                                          children: _getDynamicCustomBolusPins(drugName, visibleMinX, visibleMaxX, chartW, Colors.grey.shade800),
                                        ...
                                        child: ListView(
                                          children: [
                                            ...
                                            Builder(builder: (context) {
                                              // 💡 持続投与（Infusion）と単回投与（Bolus）を統合して時系列に並べる
                                              final List<Map<String, dynamic>> drugLogs = [];

                                              // 1. Infusion (持続投与) を追加
                                              for (var entry in _infusionMap.entries) {
                                                    ...
                                                    'widget': Padding(
                                                      ...
                                                      child: InkWell(
                                                        onTap: () => _showEditDeleteDialog(
                                                          ...
                                                          initialTime: pt.time,
                                                          id: pt.id, // 💡 IDを渡して強調表示を有効にする
                                                          initialAmount: pt.isStop ? null : pt.val,
                                                          amountLabel: '設定値',
                                                          onDelete: () => setState(() => _infusionMap[entry.key]!.removeWhere((p) => p.id == pt.id)),
                                                          onUpdate: (nt, na) => setState(() {
                                                            int idx = _infusionMap[entry.key]!.indexWhere((p) => p.id == pt.id);
                                                            if (idx != -1) {
                                                              _infusionMap[entry.key]![idx].time = nt;
                                                              if (na != null) {
                                                                _infusionMap[entry.key]![idx] = InfusionPoint(id: pt.id, time: nt, val: na, isStop: pt.isStop);
                                                              }
                                                              _infusionMap[entry.key]!.sort((a, b) => a.time.compareTo(b.time));
                                                            }
                                                          }),
                                                        ),
                                                        child: Padding(
                                                          ...
                                                          child: Text(
                                                            ...
                                                            style: TextStyle(
                                                              fontSize: 12,
                                                              fontWeight: FontWeight.bold,
                                                              color: pt.isStop ? Colors.red.shade700 : Colors.indigo,
                                                              letterSpacing: 0.2,
                                                            ),
                                                          ),
                                                        ),
                                              ...
                                              // 3. 全てのログを時間順にソート
                                              drugLogs.sort((a, b) => a['time'].compareTo(b['time']));

                                              return Column(
                                                crossAxisAlignment: CrossAxisAlignment.start,
                                                children: drugLogs.map((l) => l['widget'] as Widget).toList(),
                                              );
                                            }),
