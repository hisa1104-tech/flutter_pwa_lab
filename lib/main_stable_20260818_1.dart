      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            Widget buildDropdownSetting(String label, String value, List<String> items, Function(String?) onChanged) {
              return Padding(
            ...
            return AlertDialog(
              title: const Text('デフォルト設定', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
              content: SizedBox(
                width: 350,
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      const Text('【表示項目】', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                      Wrap(
                        children: {
                          'O2': 'O2', 'N2O': 'N2O', 'Dex': 'Dex',
                          'PropofolCiv': 'Pro持続', 'PropofolIv': 'Pro-iv',
                          'Midazolam': 'Mida', 'Acerio': 'アセリオ',
                          'Ropion': 'ロピオン', 'LA': 'LA', 'Fluid': '輸液',
                        }.entries.map((e) => SizedBox(
                          width: 110,
                          child: CheckboxListTile(
                            title: Text(e.value, style: const TextStyle(fontSize: 11)),
                            value: tempVisible.contains(e.key),
                            dense: true,
                            contentPadding: EdgeInsets.zero,
                            onChanged: (val) {
                              setDialogState(() {
                                if (val == true) tempVisible.add(e.key);
                                else tempVisible.remove(e.key);
                              });
                            },
                          ),
                        )).toList(),
                      ...
                      const Divider(),
                      const Text('【初期値の設定】', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                      buildDropdownSetting('デフォルト輸液:', tempFluid, ['フィジオ140', 'ラクテック注', 'ソルデム3A', '生理食塩水', 'ソリタT1', 'ソリタT3'], (v) => tempFluid = v!),
                      buildDropdownSetting('Propofol単位:', tempPropofolUnit, ['mg/kg/h', 'mL/h', 'μg/mL'], (v) => tempPropofolUnit = v!),
                      buildDropdownSetting('Dex単位:', tempDexUnit, ['μg/kg/h', 'mL/h'], (v) => tempDexUnit = v!),
                      buildDropdownSetting('デフォルトゲージ:', tempIvGauge, ['20G', '22G', '24G'], (v) => tempIvGauge = v!),
                      buildDropdownSetting('デフォルト穿刺部位:', tempIvSite, ['左前腕', '右前腕', '左手背', '右手背', '左肘', '右肘'], (v) => tempIvSite = v!),
                      buildDropdownSetting('デフォルト局麻剤:', tempLaDrug, ['オーラ注', 'キシロカイン', 'アナペイン', 'マーカイン'], (v) => tempLaDrug = v!),
                    ],
                  ),
                ),
              ),
