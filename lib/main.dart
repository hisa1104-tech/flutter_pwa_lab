...
class _MainRecordPageState extends State<MainRecordPage> {
  ...
  Future<void> _importFromFile() async {
    try {
      // 💡 挑戦: FileType.custom に戻し、直接ファイルブラウザを開くように試みます。
      // Web/iPad環境での MissingPluginException 回避のため、より標準的なパラメータ構成にします。
  ...
  Future<void> _importSettings() async {
    try {
      // 💡 こちらも FileType.custom に戻して余計なメニュー（写真など）を抑制します
      final result = await FilePicker.pickFiles(
        type: FileType.custom,
        allowedExtensions: ['json'],
        withData: true,
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                const Text('v1.23', style: TextStyle(color: Colors.white60, fontSize: 8.5)),
                              ],
                            ),
                            const SizedBox(height: 4),
                            Row(
