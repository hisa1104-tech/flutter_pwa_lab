...
class _MainRecordPageState extends State<MainRecordPage> {
  ...
  Future<void> _saveAllData() async {
    try {
      ...
      final success = await prefs.setString('anes_case_data', jsonStr);
      if (!success) {
        debugPrint('LocalStorageへの保存に失敗しました');
      }
    } catch (e) {
      debugPrint('保存エラー: $e');
    }
  ...
  Future<void> _loadAllData() async {
    final prefs = await SharedPreferences.getInstance();
    final jsonStr = prefs.getString('anes_case_data');
  ...
  Future<void> _clearAllData() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove('anes_case_data');
    setState(() {
      _pIdCtrl.clear();
      _pNameCtrl.clear();
      _pAgeCtrl.clear();
      ...
      _bolusLogs.clear();
      _showN2oRow = false;
      _showAcerioRow = false;
      _showRopionRow = false;
      _showDexRow = false;
      _hiddenRowKeys.clear();
      _manuallyHiddenRows.clear();
