...
// =========================================================================
  ...
  // 💡 グラフ全体の最大表示時間を、すべての記録データから網羅的に計算します。
  double _calculateTotalMaxX() {
    double maxX = 30.0;
    if (_startTime == null) return maxX;
    ...
    for (var e in _events) {
      if (e.time != null) {
        double m = e.time!.difference(_startTime!).inMinutes.toDouble();
        if (m > maxX) maxX = m;
      }
    }
    // 3. 点滴・持続投与 (OFFスタンプ含む)
    for (var key in _infusionMap.keys) {
      if (_infusionMap[key]!.isNotEmpty) {
        double m = _infusionMap[key]!.last.time.difference(_startTime!).inMinutes.toDouble();
        if (m > maxX) maxX = m;
      }
    }
    // 4. 単回投与(Bolus)
    for (var b in _bolusLogs) {
      double m = b.time.difference(_startTime!).inMinutes.toDouble();
      if (m > maxX) maxX = m;
    }
    // 5. リマークス（コメント）
    for (var rm in _remarkLogs) {
      double m = rm.time.difference(_startTime!).inMinutes.toDouble();
      if (m > maxX) maxX = m;
    }
    // 6. ルート確保(IV)
    for (var iv in _ivRecords) {
      double m = iv.time.difference(_startTime!).inMinutes.toDouble();
      if (m > maxX) maxX = m;
    }

    return maxX + 5.0; // 💡 常に5分の余白を持たせる
  }
  ...
  */
  ...
  LineChartData _mainChartData() {
    ...
    // 💡 共通メソッドで正確な最大時刻を取得
    double totalMaxMinutes = _calculateTotalMaxX();
    ...
    return Padding(
      ...
      child: InkWell(
        onTap: () => setState(() {
          ...
          if (mins > 0) {
            // 💡 共通メソッドで正確な最新時刻を取得
            double currentTotalMaxX = _calculateTotalMaxX();
      ...
      body: SafeArea(
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            // 💡 共通メソッドを使用して、画面全体の最大時刻を一元管理します
            double totalMaxX = _calculateTotalMaxX();
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                const Text('v1.26', style: TextStyle(color: Colors.white60, fontSize: 8.5)),
                              ],
                            ),
                            const SizedBox(height: 4),
                            Row(
